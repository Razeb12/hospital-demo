  ...
  <footer class="bg-sky-600 mt-20">
    <div class="mx-auto max-w-7xl overflow-hidden py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div class="mt-8 flex justify-center space-x-6">
        <a v-for="item in navigation.social" :key="item.name" :href="item.href" class="text-white hover:text-green-500">
          <span class="sr-only">{{ item.name }}</span>
          <component :is="item.icon" class="h-6 w-6" aria-hidden="true" />
        </a>
      </div>
      <p class="mt-8 text-center text-base text-white">&copy; {{currentYear}} Gynescope Specialist Hospital. All rights reserved.</p>
    </div>
  </footer>
</template>
