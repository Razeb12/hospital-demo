<template>
  <Popover class="w-full sticky top-0 z-50 bg-white shadow-lg">
    <div class="mx-auto max-w-7xl px-4 sm:px-6">
      <div class="flex items-center justify-between py-6 md:justify-start md:space-x-10">
        <div class="flex justify-start lg:w-0 lg:flex-1">
          <NuxtLink to="/">
            <span class="sr-only">Your Company</span>
            <img class="h-8 w-auto sm:h-12" :src="logo" alt />
          </NuxtLink>
        </div>
        ...
        <PopoverGroup as="nav" class="hidden space-x-10 md:flex">
          ...
          <NuxtLink v-for="item in navigation" :key="item.name" :to="item.href"
            :class="[$route.path === item.href ? 'text-green-500 border-b border-green-500' : 'text-base font-medium text-gray-500 hover:text-green-500']">
            {{ item.name }}</NuxtLink>
        ...
        <div class="divide-y-2 divide-gray-50 rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
          <div class="px-5 pt-5 pb-6">
            <div class="flex items-center justify-between">
              <div>
                <img class="h-12 w-auto" :src="logo" alt="Gynescope logo" />
              </div>
              <div class="-mr-2">
                <PopoverButton
                  class="inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500">
            ...
            <div class="mt-6">
              <nav class="grid gap-y-8">
                <a v-for="item in solutions" :key="item.name" :href="item.href"
                  ...
                  <component :is="item.icon" class="h-6 w-6 flex-shrink-0 text-sky-600" aria-hidden="true" />
          ...
          <div class="space-y-6 py-6 px-5">
            ...
            <div>
              <NuxtLink to="/contact-us"
                class="flex w-full items-center justify-center rounded-md border border-transparent bg-sky-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-indigo-700">
