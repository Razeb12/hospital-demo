          ...
          <Popover.Group as="nav" className="hidden space-x-10 md:flex">
            {navigation.map(el =>
              <Link key={el.name} href={el.href}>
                <span className={router.pathname === el.href ? "text-green-500 border-b border-green-500" : "ml-3 text-base font-medium text-gray-500 hover:text-green-500"}>
                  {el.name}
                </span>
              </Link>
            )}
                ...
                <nav class="grid gap-y-8">
                  {navigation.map(item =>
                    <Link
                      ...
                      <item.icon
                        class="h-6 w-6 flex-shrink-0 text-sky-600"
                        aria-hidden="true"
                      />
